<# 
.SYNOPSIS
Downloads the latest x3270/s3270 executables and syncs them into the `binaries/` tree.

.DESCRIPTION
This lightweight prototype targets the SourceForge release feed for the x3270 project.
It grabs the most recent 64-bit Windows “no install” ZIP, unpacks the .exe files that 
3270Connect embeds, and replaces the contents of `binaries/windows`. There is also a 
hook for a Linux archive if you produce one separately; point `-LinuxArchiveUrl` at a tarball 
that contains `x3270`, `s3270`, and `x3270if` and the script will extract those to 
`binaries/linux`. When you add `-RegenerateBindata` the script reruns `go-bindata` so the 
embedded asset set stays in sync.

Run this from the repo root (or dot-source it) and inspect the trace output for missing files.
#>
